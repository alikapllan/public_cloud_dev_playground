CLASS LHC_ZAHK_I_ERRORCODE_S DEFINITION FINAL INHERITING FROM CL_ABAP_BEHAVIOR_HANDLER.
  PRIVATE SECTION.
    METHODS:
      AUGMENT_ERRORCODE FOR MODIFY
        IMPORTING
          ENTITIES_CREATE FOR CREATE ErrorCodeAll\_ErrorCode
          ENTITIES_UPDATE FOR UPDATE ErrorCode.
ENDCLASS.

CLASS LHC_ZAHK_I_ERRORCODE_S IMPLEMENTATION.
  METHOD AUGMENT_ERRORCODE.
    DATA: text_for_new_entity      TYPE TABLE FOR CREATE ZAHK_I_ErrorCode\_ErrorCodeText,
          text_for_existing_entity TYPE TABLE FOR CREATE ZAHK_I_ErrorCode\_ErrorCodeText,
          text_update              TYPE TABLE FOR UPDATE ZAHK_I_ErrorCodeText.
    ...
    DATA: text_tky_link  TYPE STRUCTURE FOR READ LINK ZAHK_I_ErrorCode\_ErrorCodeText,
    ...
    LOOP AT entities_create INTO DATA(entity).
      ...
      ENDLOOP.
    ENDLOOP.
    MODIFY AUGMENTING ENTITIES OF ZAHK_I_ErrorCode_S
      ENTITY ErrorCode
        CREATE BY \_ErrorCodeText
        FROM text_for_new_entity
        RELATING TO entities_create BY relates_create.

    IF entities_update IS NOT INITIAL.
      READ ENTITIES OF ZAHK_I_ErrorCode_S
        ENTITY ErrorCode BY \_ErrorCodeText
